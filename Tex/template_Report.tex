...
\marginparwidth15mm
%
% definicja strony tytułowej-PROSZĘ NIE EDYTOWAĆ
...
% KONIEC DEFINICJI STRONY TYTUŁOWEJ
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%DANE DO PRACY: proszę ZMIENIĆ przykładowe dane na odpowiadające KONKRETNEJ pracy dyplomowej
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
%AUTOR PRACY
...
%RODZAJ PRACY (licencjacka lub magisterska lub inżynierska)
\def\rodzajpracy{inżynierska}
%KIERUNEK
\def\kierunek{informatyka}
%OPIEKUN czyli PROMOTOR\\INSTYTUT\\WYDZIAŁ I UCZELNIA jeżeli inne niż WFMI PK
\def\opiekun{dra hab. Jana Kucwaja\\
	Instytut Informatyki }
%MIASTO gdzie odbywa się obrona
\def\miasto{Kraków}
%ROK złożenia pracy
\def\rok{2017}
%% KONIEC DANYCH
...
\begin{figure}[ht!]
	...
	\caption{Schemat działania operatora zip \cite{marbles}.}
	\label{zip}
\end{figure}\\
...
Na rysunku \ref{aggregatingNetworkRequests} można dostrzeć, że obsłużona została częsta sytuacja, w której odpowiedzi od API obsługiwane są na wątku w tle a binding tych odpowiedzi do interfejsu użytkownika powininen odbyć się na wątku głównym. Ten problem, rozwiązany został przy użyciu operatora \textbf{observeOn}.
\begin{figure}[ht!]
	\centering
...
\begin{figure}[ht!]
	...
	\caption{Widok szczegółów  filmu/serialu \textit{[opracowanie:własne]}}
...
\begin{figure}[ht!]
	...
	\caption{Widok proponowania pozycji do obejrzenia \textit{[opracowanie:własne]}}
	\label{proposition}
\end{figure}\\
...
\begin{figure}[ht!]
	...
	\caption{Widok oceniania zapisanych w bazie produkcji \textit{[opracowanie:własne]}}
	\label{rating}
\end{figure}\\
